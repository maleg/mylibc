...
void sha256_transform(SHA256_CTX *ctx, const uint32_t data[])
{
	...
	//hexdump_bytes_hn("sha256inv_init hash : ", (uint8_t*)m, 256);
	...
	h = ctx->state[7];
	for (i = 0; i < 64; ++i) {
		//printf("Round%d : a=%.8X b=%.8X c=%.8X d=%.8X e=%.8X f=%.8X g=%.8X h=%.8X t1=%.8X t2=%.8X\n", i, a, b, c, d, e, f, g, h, t1, t2);
